...
One caveat needs attention: in first order logic, where the very behavior of the equals-sign $$=$$ is, by default, undefined, one also needs to add two more properties to the list &mdash; that $$=$$ be an **equivalence relation** (i.e. $$x=x$$, $$x=y \Rightarrow y=x$$, $$x=y \wedge y=z \Rightarrow x=z$$) and that $${{N}}$$ be **closed under** $$=$$ (i.e. $$x\in{{N}} \wedge y=x \Rightarrow y\in{{N}}$$). However, I will not split hairs to _this_ extent. Instead, over here, the equals-sign is simply the same equality used in informal set theory with all its expected properties.

Interestingly, one can define $$0$$ and the successor function $$S$$ purely using set theory: $$0=\emptyset$$ and $$S(n)=n\cup\{n\}$$ for any set $$n$$. Then one defines a set to be **inductive** if it contains $$0$$ and is closed under $$S$$. Next, one axiomatically asserts that there exists a set that is contained in every inductive set. This (unique) set is defined to be $${{N}}$$. When defined in this manner, $${{N}}$$ is denoted as $$\omega$$. The reader can verify that this definition does indeed satisfy all the Peano axioms. This set theoretic definition of the naturals has the advantage that there is no difference between the natural $$n$$ and the set of all naturals less than $$n$$: $$\text{Fin}(n)$$ &mdash; they are exactly the same set as can be verified by a quick inductive argument.
