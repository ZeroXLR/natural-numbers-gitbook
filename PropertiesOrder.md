...
**Least Element**: Let $$S\subseteq{{N}}$$. Then, an element $$n \in S$$ is called the **least element** of $$S$$ iff for any $$x \in S$$, it is the case that $$n \leq x$$.


## Lemma ULE

**Uniqueness of the Least Element**: _If $$S\subseteq{{N}}$$ has a least element $$n$$, then it is unique_.

**Proof**: Suppose $$n'$$ is some other least element of $$S$$. Then, as $$n \in S$$, $$n' \leq n$$. At the same time, since $$n$$ is also a least element and $$n' \in S$$, $$n \leq n'$$. So, $$n=n'$$ by antisymmetry.


## Theorem Well-Order

**Well Order of $${{N}}$$**: _If $$S\subseteq{{N}}$$ and $$S\neq\emptyset$$, then $$S$$ has a least element_.

**Proof**: Do induction with $${{S}}=\{n\in{{N}}:\ \text{if}\ S\subseteq{{N}}\ \text{and}\ n \in S\ \text{, then}\ S\ \text{ has a least element}\}$$.
1. $$0\in{{S}}$$ because [it is already the least natural number](#lemma-zlnn).
2. Now suppose that $$n\in{{S}}$$. One now has to show that $$S(n)\in{{S}}$$ also. To that end, consider any $$S\subseteq{{N}}$$ such that $$S(n) \in S$$. Now, if $$n \in S$$, then one is done because by the initial hypothesis, any subset with $$n$$ must have a least element. Otherwise, consider $$S'=S\cup\{n\}$$. Clearly, $$n \in S'$$, so that $$S'$$ has some least element $$l$$. Now
    * If $$l \neq n$$, then $$l \in S$$. It is easy to see in this case that $$l$$ is the least element of $$S$$. For, fix any $$s \in S$$; then certainly $$s \in S'$$ also. But then as $$l$$ is the least element of $$S'$$, $$l \leq s$$.
    * Otherwise, $$l=n$$. In this case, one can show that $$S(n)$$ is the least element of $$S$$. Well, for starters, fix any $$s \in S$$. Since $$s \in S'$$ also and $$l=n$$ is the least element of $$S'$$, $$n \leq s$$ i.e. there is some $$k\in{{N}}$$ such that $$n+k=s$$. Now, $$k \neq 0$$ as otherwise, $$s=n+0=n\notin{{N}}$$. Thus, there is a $$k'\in{{N}}$$ such that $$S(k')=k$$. So, $$s=n+k=n+S(k')=S(n)+k'$$ (using [Lemma SSwap](#lemma-sswap)). Hence, $$S(n) \leq s$$.