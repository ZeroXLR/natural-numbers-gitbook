...
* **The Family of Sets $$\text{Fin}$$**: In [RecDef1](#theorem-recdef1), choose $$\mathcal{P}{{N}}=\{\Sigma\in{{U}}:\Sigma\subseteq{{N}}\}$$, $$\emptyset$$ and the infix-function $$
\text{append}:\mathcal{P}{{N}}\times{{N}}\ni(\Sigma,n)\mapsto\Sigma\cup\{n\}\in\mathcal{P}{{N}}
$$ as inputs. Then, one gets a unique function $$\text{Fin}:{{N}}\to\mathcal{P}{{N}}$$ such that $$\text{Fin}(0)=\emptyset$$ and $$\text{Fin}(S(n))=\text{Fin}(n)\ \text{append}\ n$$. Later, I show that $$\text{Fin}(n)$$ is precisely the set of all naturals strictly less than $$n$$.