...
* Secondly, I will show that $$f'$$ is also injective. So suppose $$x \neq \tilde{x}$$ for $$x,\tilde{x}\in{{F}}(n)$$. If neither $$x$$ nor $$\tilde{x}$$ is $$x_n$$, then by the injectivity of $$f$$
$$
f'(x)=f(x) \neq f(\tilde{x})=f'(\tilde{x})
$$
Otherwise w.l.o.g. assume that $$\tilde{x}=x_n$$. Then, necessarily, $$x_n\in{{F}}(n)$$ so that $$f'(x_n)$$ makes sense. Also, in this case, necessarily, $$x \neq x_n$$ (so $$f'(x)=f(x)$$). Hence, by the injectivity of $$f$$ applied to the fact that $$x \neq n$$ (because $$x\in{{F}}(n)$$)
$$
